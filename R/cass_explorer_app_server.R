...
cass_explorer_app_server <- function( input, output, session ) {
  ...
  output$total_matches_DT_UI <- DT::renderDataTable({
    DT::datatable(data = word_count_df_r(),
  ...
  output$word_count_summarised_DT_UI <- DT::renderDataTable({
    DT::datatable(data = word_count_summarised_df_r(),
                  options = list(dom = "lipt",
                                 pageLength = 3,
                                 lengthMenu = c(3,
                                                5,
  ...
  output$column_selector_UI <- renderUI({
    shiny::tagList(
      shiny::selectInput(inputId = "text_column",
                         label = "Select text column",
                         choices = c("", names(corpus_df)),
                         selected = dplyr::if_else(is.element("text",
                                                              names(corpus_df)), "text", "")),
      shiny::selectInput(inputId = "group_by_column",
                         label = "Select date column or column used for aggregation",
                         choices = c("", names(corpus_df)),
                         selected = dplyr::if_else(is.element("date",
                                                              names(corpus_df)), "date", "")),
      shiny::selectInput(inputId = "summarise_by",
  ...
  output$moving_type_selector_UI <- selector_UI <- shiny::renderUI({
    shiny::radioButtons(inputId = "moving_type_selector",
  ...
  output$moving_selector_UI <- shiny::renderUI({
    shiny::tagList(
      shiny::numericInput(inputId = "moving_units_total",
                          label = dplyr::if_else(condition = input$summarise_by=="",
                                                 true = paste0("Calculate moving ",
                                                               input$moving_type_selector,
                                                               " over how many units?"),
                                                 false = paste0("Calculate moving ",
                                                                input$moving_type_selector,
                                                                " over how many ",
      ...
      shiny::radioButtons(inputId = "moving_length_type_selector",
                          label = paste0("Moving ",
                                         input$moving_type_selector,
      ...
      conditionalPanel(condition = "input.moving_length_type_selector == 'non-centred'&input.moving_type_selector != 'Keep as is'", {
        shiny::tagList(shiny::numericInput(inputId = "moving_units_before",
                                           label = "Before units",
                                           value = 0L),
                       shiny::numericInput(inputId = "moving_units_after",
                                           label = "After units",
                                           value = 0L))
  ...
  output$pre_submit_help_text_UI <- renderUI({
    ...
    if (is.null(input$moving_type_selector)|is.null(input$summarise_by)) return(NULL)
