...
cas_convert_db_type <- function(source_db_type,
  ...
  source_db <- cas_connect_to_db(db_type = source_db_type,
                                 read_only = TRUE)
  
  destination_db <- cas_connect_to_db(db_type = destination_db_type,
                                      read_only = FALSE)
  
  purrr::walk(
    .x = DBI::dbListTables(conn = source_db),
    .f = function(current_table_name) {
      DBI::dbReadTable(conn = source_db,
                       name = current_table_name) %>% 
        DBI::dbWriteTable(conn = destination_db,
                          name = current_table_name)
