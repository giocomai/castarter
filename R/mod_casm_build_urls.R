...
mod_casm_build_urls_ui <- function(id){
  ...
  shiny::sidebarLayout(
    sidebarPanel = shiny::sidebarPanel(
      shiny::radioButtons(inputId = ns("url_type"), label = "Type of index urls",
                          choices = c("Incremental",
                                      "Date-based"),
                          inline = TRUE),
  ...
  moduleServer( id, function(input, output, session){
    ...
    output$build_urls_parameters_ui <- shiny::renderUI({
      ...
      if (input$url_type=="Incremental") {
        shiny::tagList(
          shiny::fluidRow(
            shiny::textInput(inputId = ns("url"),
                             label = "Base url",
            ...
            shiny::textInput(inputId = ns("url_ending"),
                             label = "Url ending",
                             value = ""),
            shiny::numericInput(inputId = ns("start_page"),
                                label = "Start page",
                                value = 1),
            shiny::numericInput(inputId = ns("end_page"),
                                label = "End page",
                                value = 10),
