...
#'   include "words", "sentences", and "paragraphs".
#' @param to_lower Defaults to `FALSE`. Whether to convert tokens to lower case.
...
#'
#' @return Invisibly returns the path to the corpus.
#' @export
#'
#' @examples
#' \dontrun{
...
cas_write_corpus <- function(
  ...
  if (tif_compliant == TRUE) {
    corpus_df <- corpus_df |>
      dplyr::group_by(id) |>
      dplyr::mutate(
        doc_id = stringr::str_c(cas_options_l$website, id, sep = "-")
      ) |>
      dplyr::ungroup() |>
