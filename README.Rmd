...
`castarter2` starts with the idea that there are basically two types of pages that are commonly found when text mining. 
...
- `contents_id`-  a table with three columns, similar to the one outlined above:
  ...
  + `id`: a unique integer identifier corresponding to a unique url
  + `url`: a url
  + `index_group`: this reflect the string defined in the `index_id` table, may or may not be useful
...
By default, SQLite databases are stored in the same folder as website data, e.g. under `base_folder/project/website/`.
