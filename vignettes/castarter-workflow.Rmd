...
```{r}
library("castarter")
cas_set_options(base_folder = fs::path(fs::path_temp(),
                                       "castarter_data"),
...
```{r}
cas_extract_links(container = "h5",
...
```{r}
extractors_l <- list(title = \(x) cas_extract_html(html_document = x,
                                                 container = "h1",
