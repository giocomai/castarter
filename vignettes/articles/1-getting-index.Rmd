...
Let's say that we are interested in analysing the press releases issued by the European Parliament. After checking that no relevant `sitemap.xml` is easily accessible and there aren't relevant pointers in the `robots.txt` (see section above for details), we decide to look at the website itself. From the [home page](https://www.europarl.europa.eu/), we can see there's a link to "All press releases", that, as expected, [leads to a page](https://www.europarl.europa.eu/news/en/press-room) where all relevant articles are listed in reverse chronological order.
...
Let's put this in practice and get the 10 most recent index pages. Let's set also the `index_group` parameter to "press-room"; this isn't mandatory, but if we want to add then index pages from other sections of the website it will make things easier. 

```{r eval = TRUE}
cas_build_urls(url = "https://www.europarl.europa.eu/news/en/press-room/page/",
               start_page = 1, 
               end_page = 10,
...
```{r eval = TRUE}
cas_build_urls(glue = TRUE, 
               url = "https://www.europarl.europa.eu/news/en/press-room?minDate={here}&maxDate={here}&contentType=all",
...
end_date <- as.Date("2024-01-25")
date_format <- "%d-%m-%Y"

date_sequence <- format(x = seq.Date(from = start_date,
                                     to = end_date,
                                     by = "day"),
                        date_format)

date_sequence_plus_1 <- format(x = seq.Date(from = start_date,
                                            to = end_date,
                                            by = "day")+1,
                               date_format)
...
```{r eval = TRUE}
cas_build_urls(url = index_urls_v,
               index_group = "press-room",
               write_to_db = TRUE)
```
