...
```{r read corpus after extraction}
...
```{r castarter EU parliament setup, eval = TRUE}
library("castarter")
cas_set_options(
  base_folder = fs::path_home(
    "R",
...
```{r corpus_actual_reading, eval =TRUE, include=FALSE}
#saveRDS(object = corpus_df, file = "eu_p_corpus.rds")
corpus_df <- readRDS(file = "eu_p_corpus.rds")
```



```{r corpus_df EU parliament, eval = FALSE}
corpus_df <- cas_read_db_contents_data() |> 
  dplyr::collect()
```
...
```{r cas_count_total_words per year}
...
```{r Italian Space Agency,eval=FALSE}
library("castarter")
cas_set_options(
  base_folder = fs::path_home(
    "R",
