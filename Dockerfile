...
RUN apt-get update && apt-get install -y  git-core libcairo2-dev libcurl4-openssl-dev libgit2-dev libicu-dev libpng-dev libssl-dev libxml2-dev make pandoc pandoc-citeproc && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("ggiraph",upgrade="never", version = "0.7.10")'
...
RUN Rscript -e 'remotes::install_version("slider",upgrade="never", version = "0.2.2")'
